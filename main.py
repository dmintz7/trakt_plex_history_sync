...
def initialize_database():
	...
	if not config.dbname in str(cursor.fetchall()):
		...
		mysql_insert("CREATE OR REPLACE ALGORITHM = UNDEFINED VIEW `plex_trakt_match` AS select `pv`.`guid` AS `guid`, ifnull(`movies_imdb`.`id`, ifnull(`episodes_tmdb`.`id`, `episodes_tvdb`.`id`)) AS `trakt_id`, `pv`.`id` AS `plex_id`, ifnull(`movies_imdb`.`imdb`, ifnull(`episodes_tvdb`.`showTVDB`, `episodes_tmdb`.`showTMDB`)) AS `web_id`, `pv`.`grandparent_title` AS `grandparent_title`, `pv`.`title` AS `title`, ifnull(`episodes_tvdb`.`seasonNumber`, `episodes_tmdb`.`seasonNumber`) AS `seasonNumber`, ifnull(`episodes_tvdb`.`episodeNumber`, `episodes_tmdb`.`episodeNumber`) AS `episodeNumber`, `pv`.`viewed_at` AS `plex_view`, convert_tz(ifnull(`movies_imdb`.`watched_at`, ifnull(`episodes_tvdb`.`watched_at`, `episodes_tmdb`.`watched_at`)), 'UTC', 'America/New_York') AS `trakt_view`, timestampdiff(SECOND, `pv`.`viewed_at`, convert_tz(ifnull(`movies_imdb`.`watched_at`, ifnull(`episodes_tvdb`.`watched_at`, `episodes_tmdb`.`watched_at`)), 'UTC', 'America/New_York')) AS `time_diff` from (((`plex_views` `pv` left join `trakt_movies` `movies_imdb` on (((substr(substring_index(`pv`.`guid`, '//', 1), 20) = 'imdb:') and (`movies_imdb`.`imdb` = substr(substring_index(`pv`.`guid`, '?', 1),(22 + length(substr(substring_index(`pv`.`guid`, '//', 1), 20)))))))) left join `trakt_episodes` `episodes_tvdb` on (((substr(substring_index(`pv`.`guid`, '//', 1), 20) = 'thetvdb:') and (concat(`episodes_tvdb`.`showTVDB`, '/', `episodes_tvdb`.`seasonNumber`, '/', `episodes_tvdb`.`episodeNumber`) = substr(substring_index(`pv`.`guid`, '?', 1),(22 + length(substr(substring_index(`pv`.`guid`, '//', 1), 20)))))))) left join `trakt_episodes` `episodes_tmdb` on (((substr(substring_index(`pv`.`guid`, '//', 1), 20) = 'themoviedb:') and (concat(`episodes_tmdb`.`showTMDB`, '/', `episodes_tmdb`.`seasonNumber`, '/', `episodes_tmdb`.`episodeNumber`) = substr(substring_index(`pv`.`guid`, '?', 1),(22 + length(substr(substring_index(`pv`.`guid`, '//', 1), 20))))))));")
	cursor.close()
	conn.close()
